...
const InfiniteGridScroller = (props) => {
    let { 
        ...
        indexOffset:defaultVisibleIndexOffset, // the 0-based starting index of the list, when first loaded
    ...
    padding !?? (padding = 0)
    runway !?? (runway = 3)
    defaultVisibleIndexOffset !?? (defaultVisibleIndexOffset = 0)
    listsize !?? (listsize = 0)
    layout !?? (layout = 'uniform')
    // constraints
    defaultVisibleIndexOffset = Math.max(0,defaultVisibleIndexOffset) // non-negative
    defaultVisibleIndexOffset = Math.min(listsize, defaultVisibleIndexOffset) // not larger than list
        ...
        >
            ...
            >
                <Cradle 
                    ...
                    cellHeight = { cellHeight }
                    listsize = { listsize }
                    defaultVisibleIndexOffset = { defaultVisibleIndexOffset }
                    orientation = { orientation }
                    getItem = { getItem }
                    functions = { functions }

...
The top level component. `position:absolute`; `top`, `right`, `bottom`, `left` all set to 0. Requires a container. Responds to page `onResize`, but not to its container resize (waiting for wider deployment of [ResizeObserver](https://developer.mozilla.org/en-US/docs/Web/API/ResizeObserver))
